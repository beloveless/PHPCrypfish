...
static void layoutobfuscation(Php::Parameters &params)
    {
        ...
        if (params.size() != 1)
        {
            cout << "PHPCrypfish : You need to supply one argument to this program.\n";
        }
        else
        {
    ...
    static void obfuscation(Php::Parameters &params)
    {
        ...
        else
        {
            ...
            string hasil = obfuskasi(codeAwal);
            std::string enc_code = "<?php PHPCrypfish::decode('" + openssl_enc(type, hasil) + "'); ?>";
...
static void encrypt_file(Php::Parameters &params)
    {
        ...
        std::string enc_code = "<?php PHPCrypfish::decode('"+openssl_enc(type, code)+"'); ?>";
    ...
    static void decrypt_file(Php::Parameters &params)
    {
        ...
        std::string extract_code = Php::call("str_replace","<?php PHPCrypfish::decode('","", code); 
